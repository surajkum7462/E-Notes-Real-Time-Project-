package com.suraj.config;


import java.awt.Component;
import java.util.List;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import io.swagger.v3.oas.models.Components;
import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.info.License;
import io.swagger.v3.oas.models.security.SecurityRequirement;
import io.swagger.v3.oas.models.security.SecurityScheme;
import io.swagger.v3.oas.models.security.SecurityScheme.In;
import io.swagger.v3.oas.models.security.SecurityScheme.Type;
import io.swagger.v3.oas.models.servers.Server;

@Configuration
public class SwaggerConfig {

	/*
	 * 🧾 What is OpenAPI? OpenAPI is a specification for describing and documenting
	 * RESTful APIs. It was originally known as Swagger. The OpenAPI class you're
	 * using in your Spring Boot project (with springdoc-openapi) represents this
	 * specification in Java.
	 * 
	 * When you define an OpenAPI bean in your Spring Boot application, you're
	 * customizing how your API documentation appears in Swagger UI.
	 * 
	 */
	@Bean
	public OpenAPI openAPI() {
		OpenAPI openAPI = new OpenAPI();
		Info info = new Info();
		
		info.setTitle("Enotes API");
		info.setDescription("Enotes Api");
		info.setVersion("1.0.0");
		info.setTermsOfService("http://thebecoder.com");
		info.setContact(new Contact().email("kumarsuraj7562998828@gmail.com")
				.name("Suraj Kumar").url("http://thebecoder.com/contact"));
		info.setLicense(new License().name("Enotes 1.0").url("http://thebecoder.com/contact"));

	List<Server> serverList = List.of(new Server().description("Dev").url("http://localhost:8080"),
		new Server().description("Test").url("http://localhost:8081"),
		new Server().description("Prod").url("http://localhost:8082"));
		
	 
	// Bearer ihvg8yg58yt58gbrfbrygft54
	
	SecurityScheme securityScheme = new SecurityScheme().name("Authorization")
			                         .scheme("bearer").type(Type.HTTP)
			                         .bearerFormat("JWT").in(In.HEADER);
	
	
	Components component = new Components().addSecuritySchemes("Token", securityScheme);
	
	
		
		openAPI.setServers(serverList);
		openAPI.setInfo(info);
		openAPI.setComponents(component);
		openAPI.setSecurity(List.of(new SecurityRequirement().addList("Token")));
		return openAPI;

	}

}
